...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    requireEmailVerification: false, // flip on once Postmark sender is verified
  },

  /**
   * Each provider is independently optional. Leave a pair of credentials unset
   * and that button simply isn't offered; email+password still works with none
   * of them set. Registering a half-configured provider would break every auth
   * route, not just its own button, which is why this is a spread per provider
   * rather than one ternary over the whole object.
   */
  socialProviders: {
    ...(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET
      ...
      : {}),
    /**
     * LinkedIn, and what it is *not*.
     *
     * This is Sign In with LinkedIn using OpenID Connect, the only LinkedIn
     * product a self-serve app can provision. The scopes are fixed at
     * `openid profile email` and /v2/userinfo answers with exactly eight
     * fields: sub, name, given_name, family_name, picture, locale, email,
     * email_verified.
     *
     * There is no headline, no vanity URL, no current employer, and above all
     * no connections. Those live behind LinkedIn's partner programmes, not
     * behind a scope we could add here, so nothing downstream should be built
     * expecting this account to grow a network later. Treat it as what it is:
     * a faster way in, and a real name and photo on day one.
     */
  ...
  account: {
    accountLinking: {
      ...
      //
      // LinkedIn is deliberately absent. Its own documentation says Sign In
      // with LinkedIn "does not verify user identities and should not be
      // marketed as such", so it doesn't earn the blanket trust Google has.
      // LinkedIn sign-in still works, and still links on an address LinkedIn
      // itself marked verified; what it can't do is silently adopt an existing
      // Ryzn password account on an unverified one.
