...
export function inviteUrl({ code, name, founder, role, org, origin = inviteOrigin() }) {
  ...
  // inviting — mentor-invite.html reads it. Set when an org seats someone.
  if (org) params.set("org", org);
  if (role) params.set("role", role);
  params.set("claim", `${origin}/app/#/join?code=${encodeURIComponent(code || "")}${String(role || "").toLowerCase() === "mentee" ? "&role=mentee" : ""}`);
