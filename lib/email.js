 ...
 *
 * The real invite experience is the invite page, mentor-invite.html for the
 * roster, mentee-invite.html for students (hero, pitch, code gate); inviteUrl()
 * picks by role. This mail is only a thin pointer to that page, no code block,
 * no duplicate pitch. The link already carries code/name/founder in the query
 * string so the page opens pre-filled. Plain-text URL stays as a fallback when
 * clients strip the button.
 */
export function inviteEmail({ name, url, founder, role = "mentor" }) {
  const who = (name || "").trim();
  const from = (founder || "Bilal").trim();
  const greeting = who ? `${who},` : "Hello,";
  const asMentee = String(role || "mentor").toLowerCase() === "mentee";
  const pitch = asMentee
    ? `${from} invited you to join Ryzn as a mentee.`
    : `${from} invited you to join the founding mentor roster at Ryzn.`;
  const htmlPitch = asMentee
    ? `${from} invited you to Ryzn as a mentee. Open your invitation to claim your seat.`
    : `${from} invited you to the founding mentor roster. Open your invitation to see the seat and claim it.`;

  return {
    subject: asMentee
      ? `${from} invited you to Ryzn`
      : `${from} invited you to mentor at Ryzn`,
    text: [
      greeting,
      "",
      pitch,
    ...
    html: shell(
      ...
      `<p style="margin:0;font-size:15px;color:#5F5E5A;line-height:1.55;">${htmlPitch}</p>
...
/**
 * The org version of the same invitation. Same page, same claim flow, the only
 * difference is who is doing the inviting, and that difference matters: a seat
 * offered by their own company reads as a very different thing from one offered
 * by Ryzn, and pretending otherwise gets the mail deleted.
 */
export function orgInviteEmail({ name, url, inviter, org, role = "mentor" }) {
  const who = (name || "").trim();
  const from = (inviter || "").trim();
  const company = (org || "").trim() || "their organisation";
  const asMentor = String(role || "mentor").toLowerCase() !== "mentee";
  const seat = asMentor ? "mentor" : "mentee";
  const by = from
    ? `${from} invited you to ${company} on Ryzn as a ${seat}`
    : `You're invited to ${company} on Ryzn as a ${seat}`;
  const perk = asMentor
    ? `Claiming it joins you to ${company} and gives you your own mentor profile, roster and Impact Score.`
    : `Claiming it joins you to ${company} and opens your mentee seat, your own profile, programme and XP.`;
  ...
  return {
    ...
    html: shell(
      who ? `${who}, you've been invited to ${company}.` : `You've been invited to ${company}.`,
      `<p style="margin:0;font-size:15px;color:#5F5E5A;line-height:1.55;">${by}. ${perk}</p>
       <a href="${url}" style="display:block;background:#5B4FCF;color:#FFFFFF;font-size:15px;font-weight:700;text-decoration:none;text-align:center;border-radius:12px;padding:15px 20px;margin:24px 0 12px;">Open your invitation &rarr;</a>
       <p style="margin:0;font-size:12px;color:#5F5E5A;line-height:1.5;">Or paste this into your browser:<br /><span style="color:#5B4FCF;word-break:break-all;">${url}</span></p>`
    ),
  };
