 ...
 */
export function orgInviteEmail({ name, url, inviter, org }) {
  const who = (name || "").trim();
  const from = (inviter || "").trim();
  const company = (org || "").trim() || "their organisation";
  const by = from ? `${from} invited you to mentor at ${company}` : `You're invited to mentor at ${company}`;
  ...
  return {
    ...
    text: [
      who ? `${who},` : "Hello,",
      "",
      `${by} — mentoring runs on Ryzn, so the seat comes with your own profile, roster and Impact Score.`,
      "",
      "Open your invitation:",
      url,
      "",
    ...
    html: shell(
      who ? `${who}, ${company} saved you a seat.` : `${company} saved you a seat.`,
      `<p style="margin:0;font-size:15px;color:#5F5E5A;line-height:1.55;">${by}. Claiming it joins you to ${company} on Ryzn and gives you your own mentor profile, roster and Impact Score.</p>
       <a href="${url}" style="display:block;background:#5B4FCF;color:#FFFFFF;font-size:15px;font-weight:700;text-decoration:none;text-align:center;border-radius:12px;padding:15px 20px;margin:24px 0 12px;">Open your invitation &rarr;</a>
       <p style="margin:0;font-size:12px;color:#5F5E5A;line-height:1.5;">Or paste this into your browser:<br /><span style="color:#5B4FCF;word-break:break-all;">${url}</span></p>`
    ),
  };
