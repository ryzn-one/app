...
/**
 * The public face of a post: ryzn.one/p/<slug>.
 *
 * Everything else about a post is gated — a share link used to be
 * /app/#/post/<id>, which is a deep link into an authenticated SPA. Pasted into
 * Slack or iMessage it unfurled as the marketing site ("Ryzn — Rise now.")
 * because the crawler only ever sees site/index.html, and anyone without an
 * account hit the sign-in wall. So the link proved nothing and travelled nowhere.
 *
...
/** The link itself, given an origin. One place so the API and the page agree. */
export const shareUrlFor = (origin, slug) => `${origin}/p/${slug}`;
...
export function renderPostPage({ post, author, origin }) {
  const url = shareUrlFor(origin, post.slug);
