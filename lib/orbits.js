 ...
 *   domainJoin  who walks in off a verified email domain
 *
 * Seven fields, and every behavioural difference between the three orbit kinds
 * is one of them. Adding an eighth is how the product stays one codebase;
 * adding a `kind` check is how it stops being one.
 */
...
/**
 * `domainJoin`, the seventh switch.
 *
 *   off      nobody is seated by their address
 *   suggest  they're offered the orbit and tap once to take it
 *   auto     they're seated on their next boot and told so
 *
 * It only ever fires on a domain the orbit has proved it owns by DNS, and only
 * for a member whose own address is verified — see lib/domains.js, which owns
 * all three gates. The switch is the third of them, and it lives here rather
 * than beside the domain list because it is a rule about who is in an orbit,
 * which is exactly what this object is for.
 *
 * Clamped to "off" outside a private orbit by `SCOPES_FOR_KIND`'s sibling below:
 * a circle is joined by its link and Ryzn's own orbit already holds everyone,
 * so in both of them this would be a switch with nothing behind it.
 */
export const DOMAIN_JOIN_MODES = new Set(["off", "suggest", "auto"]);
 ...
 *
 * `domainJoin: "suggest"` is the one default that leans open, and it is safe to
 * because it is inert until an admin has proved a domain by DNS — an org that
 * has claimed nothing has nothing for this to match. Once they have, the first
 * behaviour a new employee meets is an offer they tap, not a room they were put
 * in; an org that wants the room turns it up to "auto" themselves.
 */
...
/**
 * Which `domainJoin` modes an orbit kind can mean.
 *
 * Only a company orbit has an employer behind it whose domain could be proved,
 * so the other two are clamped to a single legal value rather than left to a
 * console to set and nothing to enforce. Same shape as `SCOPES_FOR_KIND`
 * directly above, and for the same reason.
 */
const DOMAIN_JOIN_FOR_KIND = {
  public: ["off"],
  community: ["off"],
  private: ["off", "suggest", "auto"],
};
...
export function resolvePolicy(orbit) {
  ...
  const joinModes = DOMAIN_JOIN_FOR_KIND[kind];
  ...
  return {
    ...
    domainJoin: joinModes.includes(stored.domainJoin) ? stored.domainJoin : base.domainJoin,
  };
}
...
export function cleanPolicy(input, orbit) {
  ...
  return {
    ...
    domainJoin: joinModes.includes(patch.domainJoin) ? patch.domainJoin : current.domainJoin,
  };
}
