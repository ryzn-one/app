 ...
 *
 *   GET                          list codes, newest first, with who claimed them
 *   POST  { count, expiresDays, note }   mint single-use codes
 ...
 * anyone — minting a code and claiming it stay separate operations.
 */
...
async function handler(request, admin) {
  ...
  if (request.method === "POST") {
    ...
    const count = Math.min(Math.max(Number(body.count) || 1, 1), MAX_MINT);
    const days = Number(body.expiresDays) > 0 ? Number(body.expiresDays) : 90;
    const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
    const note = typeof body.note === "string" && body.note.trim() ? body.note.trim().slice(0, 120) : "Founding cohort";
    ...
    return json({ created: docs.map((d) => d.code), expiresAt }, 201);
  }
